...
namespace NieR_Replicant_Viet_Hoa
{
    public class Unpacker
    {
        ...
        private string ReadString(ref BinaryReader reader)
        {
            ...
            while (ch != 0 && reader.BaseStream.Position < reader.BaseStream.Length)
            {
                str.Append(Encoding.ASCII.GetString(new byte[] { ch }));
                ch = reader.ReadByte();
            }
            return str.ToString();
        }
        private BXON_Archive[] GetArchives(ref BinaryReader reader)
        {
            ...
            if (header.Magic != 0x4E4F5842) throw new Exception(Default._Messages["UnsupportedFormat"]);
        ...
        public void Unpack(string des, string infoDir, ProgressManager progress, LogManager log)
        {
            progress.Begin();
            string info = Path.Combine(_RootDirectory, infoDir, "info.arc");
            if (!File.Exists(info)) throw new Exception(Default._Messages["NotFound"]);
            ...
            else throw new Exception(Default._Messages["UnsupportedFormat"]);
            ...
            log.Push(Default._Messages["UnpackData"].Replace("{Item}", Path.GetFileName(infoDir).ToUpper()).Replace("{FileCount}", fileCount.ToString()));
